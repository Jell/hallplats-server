module Provider::Vasttrafik
  ...
  def update_stop_database
    ...
    xml_tree.css("all_stops item").each do |element|
      ...
      bus_stop = BusStop.find_or_initialize_by_stop_id_and_provider_name(stop_id, provider_name)
  ...
  def get_forecast_url(poi_id)
    use_identifier
    url = "http://www.vasttrafik.se/External_Services/NextTrip.asmx/GetForecast?identifier=" + @identifier + "&stopId=" + poi_id
  end
  ...
  def parse_forecast(response, poi_id, poi_name)
    ...
    xml_tree.css("forecast item").each do |element|
      ...
      alternate_text = element.attribute("next_trip_alternate_text").text.to_s
      ...
      attributes[:next_next_trip]       = element.attribute("next_next_trip").text.to_s
      alternate_text = element.attribute("next_next_trip_alternate_text").text.to_s
  ...
  private
  def fetch_all_stops
    get_poi_list_url = "http://www.vasttrafik.se/External_Services/TravelPlanner.asmx/GetAllStops?identifier=" + @identifier
    xml_tree = fetch_xml_from_url_string(get_poi_list_url)
  end

  def fetch_forecast(poi_id)
    get_poi_forecast_url = "http://www.vasttrafik.se/External_Services/NextTrip.asmx/GetForecast?identifier=" + @identifier + "&stopId=" + poi_id
    xml_tree = fetch_xml_from_url_string(get_poi_forecast_url)
  end

  def use_identifier
    @identifier = "3e383cd8-30fa-47dc-8379-7d4295dc9db2"
  end
end