...
receive do |f|
  f.when([:parse, Array]) do |array|
    ...
    while array[i] do
      ...
      loader = BusStopLoader.new("Provider::#{provider_name}".constantize)
      parsed = loader.parse_forecast(tobeparsed.force_encoding("UTF-8"), stop_id, element[:name].force_encoding("UTF-8"))
