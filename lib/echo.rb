...
receive do |f|
  f.when([:parse, Array]) do |array|
    #url = "http://www.vasttrafik.se/External_Services/NextTrip.asmx/GetForecast?identifier=3e383cd8-30fa-47dc-8379-7d4295dc9db2&stopId=#{result[:stop_id]}"
    result = []
    i = 0
    while array[i] do
      element = {}
      ...
      loader = BusStopLoader.new("Provider::#{element[:provider_name]}".constantize)
      parsed = loader.parse_forecast(tobeparsed.force_encoding("UTF-8"), element[:poi_id], element[:name].force_encoding("UTF-8"))
